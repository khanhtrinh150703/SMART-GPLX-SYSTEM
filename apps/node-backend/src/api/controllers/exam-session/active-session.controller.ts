...
export interface IActiveSessionControllerCradle {
  /** @description Dịch vụ thực hiện các thao tác ghi, kiểm tra, thu hồi và quản lý vòng đời của phiên làm việc (Command Side). */
...
export class ActiveSessionController {
  /** @private @readonly @description Instance xử lý logic nghiệp vụ và thay đổi trạng thái của phiên hoạt động. */
  private readonly _activeSessionService: IActiveSessionService;

  /** @private @readonly @description Instance xử lý logic truy vấn và trích xuất dữ liệu trạng thái phiên hoạt động. */
  private readonly _activeSessionQueryService: IActiveSessionQueryService;
  ...
  }: IActiveSessionControllerCradle) {
    this._activeSessionService = activeSessionService;
    this._activeSessionQueryService = activeSessionQueryService;
  }
   ...
   */
  public getCurrentSession = catchAsync(
    async (req: IAuthRequest, res: Response) => {
      const userId = req.user.userId;
      const result =
        await this._activeSessionQueryService.getCurrentSession(userId);
      Result.ok(
        res,
        result,
  ...
  public startSession = catchAsync(async (req: IAuthRequest, res: Response) => {
     ...
     */
    const result = await this._activeSessionService.startSession(
      userId,
      dto,
      isGuest,
    );
   ...
   * @access Private (User)
   */
  public deleteSession = catchAsync(
    async (req: IAuthRequest, res: Response) => {
