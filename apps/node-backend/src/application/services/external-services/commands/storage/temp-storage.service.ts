...
import { ITempStorageService } from "@/domain/interfaces/services/external/commands";
...
export class TempStorageService implements ITempStorageService {
  ...
  public async createTempDir(jobId: string): Promise<void> {
    ...
    await fs.mkdir(dir, { recursive: true }).catch(() => {
      throw new AppError(ErrorCode.IMPORT.DIR_CREATION_FAILED);
    });
  }
  ...
  public async saveChunk(
    ...
    await fs.writeFile(chunkPath, buffer).catch(() => {
      throw new AppError(ErrorCode.IMPORT.CHUNK_SAVE_FAILED);
    });
  }
    ...
    try {
      for (let i = 0; i < totalChunks; i++) {
        ...
        try {
          ...
          throw new AppError(
            ErrorCode.IMPORT.INVALID_CHUNK_INDEX,
      ...
      throw new AppError(ErrorCode.IMPORT.MERGE_FAILED, "Merge process failed");
    }
  }
