...
export class QuestionQueryService implements IQuestionQueryService {
   ...
   */
  public async getQuestionsByChapter(
    chapterId: string,
  ): Promise<IQuestionResponseDTO[]> {
   ...
   * @throws {AppError} QUESTION.NOT_FOUND nếu không tìm thấy đủ số lượng ID duy nhất được yêu cầu.
   */
  public async getByLicenseCategoryIds(ids: string[]): Promise<Question[]> {
    // 1. Chặn trường hợp mảng rỗng
    if (!ids || ids.length === 0) return [];
    ...
    // 3. Gọi Repository lấy danh sách Entities dựa trên danh sách ID duy nhất
    const entities =
      await this._questionRepo.findByLicenseCategoryIds(uniqueIds);

    // 4. Kiểm tra tính toàn vẹn dựa trên UNIQUE IDs (Lỗi logic 1 - Fix)
    // Phải so sánh với uniqueIds.length thay vì ids.length gốc
    if (entities.length === 0) {
      throw new AppError(ErrorCode.QUESTION.EMPTY_BANK);
    }
  ...
  /**
   * @description Lấy danh sách câu hỏi thuộc về các chương học theo danh sách IDs chương.
   * @param {string[]} chapterIds - Danh sách các ID chương học cần truy vấn câu hỏi.
   * @returns {Promise<Question[]>} Danh sách thực thể câu hỏi thuộc các chương đó.
   * @throws {AppError} QUESTION.EMPTY_BANK nếu không tìm thấy bất kỳ câu hỏi nào thuộc các chương này.
   */
  public async getByChapterIds(chapterIds: string[]): Promise<Question[]> {
    // 1. Chặn trường hợp mảng rỗng
    if (!chapterIds || chapterIds.length === 0) return [];

    // 2. Xử lý logic trùng lặp ID chương học để tối ưu hiệu năng quét DB
    const uniqueChapterIds = Array.from(new Set(chapterIds));

    // 3. Gọi Repository lấy danh sách câu hỏi dựa trên mảng ID chương duy nhất
    const entities =
      await this._questionRepo.findByChapterIds(uniqueChapterIds);

    // 4. Kiểm tra tính toàn vẹn (Nếu rỗng hoàn toàn thì báo trống ngân hàng đề)
    if (entities.length === 0) {
      throw new AppError(ErrorCode.QUESTION.EMPTY_BANK);
    }
  ...
  public async getPaginatedQuestions(
    ...
    const [questions, total] = await this._questionRepo.findAndCountAdmin(
      query,
      skip,
      limit,
    );
   ...
   */
  public async getQuestionsSummary(
