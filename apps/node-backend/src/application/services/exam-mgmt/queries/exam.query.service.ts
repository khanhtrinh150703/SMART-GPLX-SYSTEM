...
export class ExamQueryService implements IExamQueryService {
  ...
  public async getPaginatedExams(
    query: ExamQueryDTO,
  ): Promise<PaginatedResult<IExamResponseDTO>> {
    ...
    const [exams, total] = await this._examRepo.findAndCount(
      query,
      skip,
      limit,
    );
   ...
   */
  public async getPaginatedVisualExams(
    query: ExamQueryDTO,
  ): Promise<PaginatedResult<IExamSummaryResponseDTO>> {
    ...
    const examResponses = exams.map((exam) =>
      ExamMapper.toSummaryResponseDTO(exam),
    );
   ...
   * @throws {AppError} EXAM.ID_REQUIRED - Khi định danh (ID) của lượt thi bị thiếu hoặc không hợp lệ trong yêu cầu. (Exam attempt ID is required).
   * @throws {AppError} EXAM.NOT_FOUND - Khi không tìm thấy dữ liệu về lượt làm bài thi yêu cầu trong hệ thống. (Exam attempt not found).
   */
   ...
   * @throws {AppError} EXAM.ID_REQUIRED - Khi ID bị thiếu.
   * @throws {AppError} EXAM.NOT_FOUND - Khi không tìm thấy bộ đề.
   */
  public async getExamDetail(
    id: string,
  ): Promise<ExamUserFullContentResponseDTO> {
   ...
   * @returns {Promise<ExamEntity>}
   * @throws {AppError} EXAM.NAME_REQUIRED - Khi tên của lượt thi bị thiếu hoặc không hợp lệ trong yêu cầu. (Exam name is required).
   * @throws {AppError} EXAM.NOT_FOUND - Khi không tìm thấy dữ liệu về lượt làm bài thi yêu cầu trong hệ thống. (Exam attempt not found).
   */
  public async getExamByName(name: string): Promise<ExamEntity> {
    // 1. Kiểm tra tên đầu vào có hợp lệ không
    if (!name) {
      throw new AppError(ErrorCode.EXAM.NAME_REQUIRED);
    }

    // 2. Gọi Repository để truy vấn dữ liệu theo tên
    const exam = await this._examRepo.findByNameSystem(name);
