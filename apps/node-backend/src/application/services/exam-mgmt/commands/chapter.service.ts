...
import { IChapterService } from "@/domain/interfaces/services/exam-mgmt/commands/i-chapter.service";
...
export class ChapterService implements IChapterService {
  ...
  ): Promise<IChapterResponseDTO> {
    // 1. Thực thi song song các truy vấn kiểm tra (Parallel Queries)
    const [existingName, existingCode] = await Promise.all([
      this._chapterRepo.findByName(dto.name.trim()),
      this._chapterRepo.findByCode(dto.code.trim()),
    ]);

    // 2. Kiểm tra kết quả và ném lỗi (Validation logic)
    if (existingName) {
  ...
  public async updateChapter(
    ...
    // 1. Kiểm tra trùng Tên (Chỉ tìm nếu tên thay đổi)
    if (dto.name && dto.name !== chapter.name) {
      checkTasks.push(
        (async () => {
          const existing = await this._chapterRepo.findByName(dto.name);
          if (existing)
            throw new AppError(ErrorCode.CHAPTER.NAME_ALREADY_EXISTS);
        })(),
      );
    ...
    // 2. KIẾM TRA TRÙNG MÃ (Sửa lỗi: Chỉ tìm nếu mã thay đổi)
    ...
    // Thực thi song song các tác vụ kiểm tra (Parallel I/O)
  ...
  public async deleteChapter(id: string): Promise<IDeleteResponseDTO> {
    ...
    const totalRelated = related.questions + related.matrixDetails;
