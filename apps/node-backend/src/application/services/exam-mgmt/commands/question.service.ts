...
import { IQuestionService } from "@/domain/interfaces/services/exam-mgmt/commands/i-question.service";
...
} from "@/application/dtos/request/question/update-question.request.dto";
import {
  IDeleteResponseDTO,
  DeleteResponseDTO,
...
export class QuestionService implements IQuestionService {
  ...
  public async createFromImport(
    importEntity: QuestionImportEntity,
  ): Promise<void> {
  ...
  public async deleteQuestion(id: string): Promise<IDeleteResponseDTO> {
    ...
    // const totalRelated =
    //   related.chapter + related.examQuestions + related.licenseLinks;
    const totalRelated = related.examQuestions + related.questionStats;
    ...
    if (totalRelated > 0) {
      ...
      // TRƯỜNG HỢP 2: DỮ LIỆU SẠCH -> XÓA VĨNH VIỄN 
  ...
  private async _validateRelations(
    ...
    const licenseChecks = licenseIds.map((lid) =>
      this._cacheService.existsCategory(lid),
    );
