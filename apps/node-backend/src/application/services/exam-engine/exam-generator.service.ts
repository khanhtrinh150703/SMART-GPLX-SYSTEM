...
export class ExamGeneratorService implements IExamGeneratorService {
    ...
    */
    public async generate(dto: GenerateExamDTO): Promise<IExamResponseDTO> {
        // 1. Lấy Matrix và thông tin Hạng bằng lái (Parallel để tối ưu Latency)
        ...
        const subLicenses = Array.from(new Set([
            category.name,
            ...(LICENSE_HIERARCHY[category.name] || [])
        ]));

        // 3. Lấy kho câu hỏi và thực hiện Picker Logic
        const questionEntities = await this._questionQueryService.getByLicenseCategory(subLicenses);

        // PickerService trả về mảng Question Entity đã được chọn lọc theo Matrix
