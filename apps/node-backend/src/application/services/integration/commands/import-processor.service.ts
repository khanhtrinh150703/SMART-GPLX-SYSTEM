...
import { IImportProcessorService } from "@/domain/interfaces/services/integration/commands/i-import-processor.service";
...
import { QuestionService } from "../../exam-mgmt/commands";
...
export class ImportProcessorService implements IImportProcessorService {
   ...
   */
  private _mapMasterDataIds(rowData: IRawQuestion): {
    categoryIds: string[];
    chapterId: string;
  } {
    // 1. Xử lý Hạng bằng lái (License Categories)
    const categoryIds: string[] = [];
    const missingCategories: string[] = [];

    rowData.licenseCategory.forEach((name) => {
      const category = this._cacheService.getCategoryByExcelName(name);
      if (category) {
    ...
    // Nếu có hạng bằng nào không khớp, ném lỗi ngay
    if (missingCategories.length > 0) {
      throw new RowValidationError(
        `Dữ liệu không hợp lệ: Không tìm thấy hạng bằng [${missingCategories.join(", ")}]`,
        ["Hạng bằng lái"], // Chỉ định cột lỗi
        "Vui lòng kiểm tra lại tên hạng bằng trong file Excel, đảm bảo khớp chính xác với danh mục trên hệ thống.",
      );
    }

    // 2. Xử lý Chương học (Chapter)
    const chapter = this._cacheService.getChapterByExcelCode(rowData.chapter);

    if (!chapter) {
      throw new RowValidationError(
        `Dữ liệu không hợp lệ: Mã chương [${rowData.chapter}] không tồn tại`,
        ["Chương"], // Tham số này giữ nguyên mảng [] - Đã đúng
        `Vui lòng kiểm tra lại cột Chương. Đảm bảo mã chương (ví dụ: CH01, CH02...) đã được khởi tạo trong hệ thống.`,
      );
    }

    return { categoryIds, chapterId: chapter.id };
  }
   ...
   * @returns {Promise<void>}
   */
  private async _prepareImportData(
    job: ImportJobEntity,
    zipPath: string,
    extractedDir: string,
  ): Promise<void> {
