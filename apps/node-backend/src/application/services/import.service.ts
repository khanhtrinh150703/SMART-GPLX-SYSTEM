...
import { CompleteImportRequestDto, InitImportRequestDto, UploadChunkRequestDto } from '../dtos/request/import/import.dto';
...
import { IImportJobResponseDTO } from '../dtos/response/import/import-job.dto.respone';
...
import { IImportJobStatusResponseDTO } from '../dtos/response/import/import-status-response.dto';
...
export class ImportService implements IImportService {
    ...
    private readonly _importQueue: IImportQueue // Khai báo nó ở đây!
    ...
    public async initSession(dto: InitImportRequestDto): Promise<IImportJobResponseDTO> {
        ...
        const calculatedTotalChunks = Math.ceil(dto.totalSize / IMPORT_CONFIG.CHUNK_SIZE_LIMIT);

        const entity = new ImportJobEntity({
            fileName: dto.fileName,
            totalSize: dto.totalSize,
            totalChunks: calculatedTotalChunks,
            chunkSizeLimit: IMPORT_CONFIG.CHUNK_SIZE_LIMIT,
        });

        // 2. Lưu trữ vào Database (Dịch: Persist to Database via Repository)
        const savedJob = await this._importRepo.save(entity);
