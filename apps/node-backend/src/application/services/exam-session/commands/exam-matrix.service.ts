...
import { IExamMatrixService } from "@/domain/interfaces/services/exam-session/commands/i-exam-matrix.service";
...
export class ExamMatrixService implements IExamMatrixService {
   ...
   */
  public async create(
    dto: CreateExamMatrixRequestDTO,
  ): Promise<IExamMatrixResponseDTO> {
    // 1. Kiểm tra trùng tên ma trận trong hệ thống (Chặn đứng lỗi trùng DB sớm - Fail Fast)
    const existingMatrix = await this._examMatrixRepo.findByNameSystem(
      dto.name,
    );
    if (existingMatrix) {
      throw new AppError(ErrorCode.MATRIX.NAME_ALREADY_EXISTS);
    }

    // 2. Kiểm tra danh mục hạng bằng lái trong Cache
    const result = this._cacheService.getCategoryById(dto.licenseCategoryId);
    if (!result) {
      throw new AppError(ErrorCode.LICENSE.NOT_FOUND);
    }

    // 3. Kiểm tra các chương học trong Cache
    ...
    // 4. Khởi tạo Domain Entity (Tự động thực thi logic validate % tại đây)
    ...
    // 6. Trả về DTO thông qua Mapper cho Client
  ...
  public async update(
    id: string,
    dto: UpdateExamMatrixRequestDTO,
  ): Promise<IExamMatrixResponseDTO> {
    // 1. Kiểm tra sự tồn tại và lấy dữ liệu cũ trước (Fail-Fast)
    const existingEntity = await this._examMatrixRepo.findById(id);
    if (!existingEntity) {
      throw new AppError(ErrorCode.MATRIX.NOT_FOUND);
    }

    // 2. Nếu có cập nhật tên mới, kiểm tra xem tên đó có bị trùng với ma trận KHÁC không
    if (dto.name) {
      const duplicateNameMatrix = await this._examMatrixRepo.findByNameSystem(
        dto.name,
      );

      // Nếu tìm thấy ma trận trùng tên trùng và ma trận đó có ID KHÁC với ID đang cập nhật
      if (duplicateNameMatrix && duplicateNameMatrix.id !== id) {
        throw new AppError(ErrorCode.MATRIX.NAME_ALREADY_EXISTS);
      }
    }
    ...
    // 4. Cập nhật cấu hình Domain Entity bằng cách trộn (merge) dữ liệu cũ và mới
    ...
    // 5. Giao cho Repository lưu trữ toàn vẹn trạng thái mới vào DB
    const result = await this._examMatrixRepo.updateExamMatrix(existingEntity);
    this._cacheService.refresh();

    // 6. Map Entity sang DTO để phản hồi về phía Client
   ...
   */
  private async toResponse(data: ExamMatrix): Promise<IExamMatrixResponseDTO> {
    if (!data) {
      throw new AppError(ErrorCode.MATRIX.NOT_FOUND);
    }
