...
import { IExamMatrixQueryService, IMasterDataCacheService } from "@/domain/interfaces/services";
...
export class ExamMatrixQueryService implements IExamMatrixQueryService {
    ...
    constructor({ examMatrixRepository, masterDataCacheService }: IExamMatrixQueryServiceCradle) {
     ...
     */
    public async getExamMatrixSelections(): Promise<IExamMatrixSelectionResponseDTO[]> {
    ...
    public async getPaginatedExamMatrices(query: ExamMatrixQueryDTO): Promise<PaginatedResult<ExamMatrixResponseDTO>> {
        ...
        const [matrices, total] = await this._matrixRepo.findAndCount(query, skip, limit);
