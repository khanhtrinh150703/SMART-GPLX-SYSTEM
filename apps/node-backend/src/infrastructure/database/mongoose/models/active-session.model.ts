...
const ActiveSessionAnswerSchema = new Schema(
  {
    ...
    selectedAnswerIndex: { type: String, default: null },
    updatedAt: { type: Date, default: Date.now },
  },
  { _id: false },
); // Không cần sinh _id riêng cho từng item trong mảng
...
const ActiveSessionSchema = new Schema<IActiveSessionPersistence>(
  {
    ...
    userId: { type: String, required: true, index: true, unique: true }, // Đánh index để query theo User cực nhanh
 ...
 */
export const ActiveSessionModel = model<IActiveSessionPersistence>(
  "ActiveSession",
  ActiveSessionSchema,
);
