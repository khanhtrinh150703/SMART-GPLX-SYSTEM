...
interface AvatarUploadProps {
  // value bây giờ có thể là URL từ server (string) hoặc File mới chọn
  value?: string | File; 
...
export const AvatarUpload = ({ value, onChange, disabled }: AvatarUploadProps) => {
  ...
  const [preview, setPreview] = useState<string>(""); // State để giữ URL hiển thị
  const user = useUserStore((state) => state.user);

  // Xử lý logic hiển thị ảnh Preview
  useEffect(() => {
    if (!value) {
      setPreview("");
      return;
    ...
    if (value instanceof File) {
      // Nếu là File mới chọn, tạo object URL
      const objectUrl = URL.createObjectURL(value);
      setPreview(objectUrl);

      // Cleanup để tránh rò rỉ bộ nhớ (Memory leak)
      return () => URL.revokeObjectURL(objectUrl);
    } else {
      // Nếu là string (URL từ backend)
      setPreview(value);
    }
  }, [value]);
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Truyền trực tiếp đối tượng File về cho Form/Parent
      onChange(file); 
    } finally {
