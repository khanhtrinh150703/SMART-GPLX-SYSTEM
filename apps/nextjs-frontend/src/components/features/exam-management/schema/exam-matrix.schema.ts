...
export const examMatrixDetailSchema = z.object({
    ...
    percentage: z.number().min(0, 'Tỉ trọng không được âm').max(100, 'Tỉ trọng tối đa 100%'),
});

export const examMatrixSchema = z.object({
    ...
    details: z.array(examMatrixDetailSchema).min(1, 'Cần ít nhất một chương'),
})
    ...
    .refine((data) => {
        const total = data.details.reduce((sum, item) => sum + item.percentage, 0);
        return total === 100;
    }, {
