...
export const examMatrixApi = {
   ...
   */
  getAll: async (params: QueryParams): Promise<StandardResponse<PaginatedResult<IExamMatrixResponse>>> => {
    const response = await axiosClient.get(ENDPOINTS.EXAM_MATRICES.BASE, { params });
   ...
   */
  create: async (data: IExamMatrixRequest): Promise<StandardResponse<string>> => {
   ...
   */
  getById: async (id: string): Promise<StandardResponse<IExamMatrixResponse>> => {
   ...
   */
  update: async (id: string, data: IExamMatrixRequest): Promise<StandardResponse<void>> => {
    const response = await axiosClient.put(ENDPOINTS.EXAM_MATRICES.DETAILS(id), data);
    return response.data;
  },
  ...
  delete: async (id: string): Promise<StandardResponse<void>> => {
    const response = await axiosClient.delete(ENDPOINTS.EXAM_MATRICES.DETAILS(id));
    return response.data;
  },
  ...
  restore: async (id: string): Promise<StandardResponse<void>> => {
    const response = await axiosClient.patch(ENDPOINTS.EXAM_MATRICES.RESTORE(id));
    return response.data;
  },
};