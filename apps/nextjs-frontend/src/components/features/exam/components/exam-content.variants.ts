...
export const actionButtonVariants = cva(
  "h-12 rounded-2xl font-bold text-[13px] uppercase tracking-wide transition-all active:scale-95 flex items-center gap-2",
  {
    variants: {
      intent: {
        manual:
          "px-6 border-slate-200 text-slate-600 hover:border-emerald-200 hover:text-emerald-600 border",
...
export const examMatrixToolbarVariants = {
  ...
  root: cva(
    "flex flex-col w-full animate-in fade-in slide-in-from-top-4 duration-700",
  ),
