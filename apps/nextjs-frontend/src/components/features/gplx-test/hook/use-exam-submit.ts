...
import {
  ICompleteExamRequestDTO,
  IExamAnswerItem,
} from "../types/exam-complete.types";
...
interface UseExamSubmitProps {
  ...
  shouldShuffle: boolean; // Thêm vào đây
}
...
export const useExamSubmit = ({
  ...
  const handleSubmit =
    useCallback(async (): Promise<IExamUserResultResponseDTO | null> => {
      try {
        // Lấy snapshot mới nhất từ Store để tránh closure stale
        const state = useExamStore.getState();
        const { answers, timeRemaining, timeSpentPerQuestion, sessionId } =
          state;
        ...
        if (answeredCount === 0) {
          alert(
            "Bạn chưa chọn bất kỳ câu trả lời nào. Vui lòng làm bài trước khi nộp!",
          );
          return null;
        }
        ...
        const formattedAnswers: IExamAnswerItem[] = Object.entries(answers).map(
          ([qId, ansIndex]) => ({
            questionId: qId,
            answer: ansIndex,
            timeSpent: timeSpentPerQuestion[qId] || 0,
          }),
        );
    ...
    }, [limitMinutes, examActions, questions, examId, shouldShuffle]);
